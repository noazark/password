<template lang="html">
  <div class="distribution">
    cycles: {{cycles}}
    <br>
    <label><input type="checkbox" @change="doSort = $event.target.checked" :checked="doSort"></input> sort</label>
    <br>
    <br>
    variations: {{variations}} / {{Math.pow(values, length)}}
    <br>
    <ul>
      <li v-for="distribution in distributions.reverse()">
...
export default {
  ...
  methods: {
    run () {
      ...
      if (!this.runs.hasOwnProperty(pw)) {
        this.runs[pw] = 0
      }
